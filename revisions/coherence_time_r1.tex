...
This is a \textbf{lower bound}: the effective commit interval must be at least as long as the slowest constraint. When these constraints act on largely independent subsystems and can be satisfied in parallel, the bound is approximately tight and the max operation gives the dominant timescale. The four terms are:
...
\tau_{\mathrm{Landauer}} &\sim 10^{-12} \text{ s} \quad \text{(thermal relaxation)} \\
...
\paragraph{Kuramoto coherence ($r$).} Extract instantaneous phase $\theta_j(t)$ from bandpass-filtered LFP/EEG via Hilbert transform. Compute the global order parameter:
\[
r(t) = \left|\frac{1}{N}\sum_{j=1}^N e^{i\theta_j(t)}\right|
\]
where $N$ is the number of recording channels. For the coherence time model, we need module-level coherence: first cluster channels into $M$ modules, compute each module's aggregate phase, then estimate $r$ and $\kappa$ from the inter-module phase distribution. Typical values: spontaneous cortex $\sim 0.2$--0.4; attention $\sim 0.5$--0.7; circadian networks $\sim 0.7$--0.9~\cite{fries2015}.
...
Take $M=8$ occipital modules, $r=0.7$ (attention~\cite{fries2015}), full tolerance $\varepsilon=\pi$ rad, and phase-exploration rate $\Delta\omega=2\pi\times 20$ rad/s.
...
\emph{Motor loop} (cerebellar/basal ganglia): Low-$M$ ($\sim 3$--5 modules) primitives. Commits faster (50--150 ms). Arousal modulates decision threshold, preserving reaction time.
