...
The alignment probability has a precise geometric form. For $M$ independent phases, the probability that all lie within $\varepsilon$ of each other is:
...
\textbf{The exponent is $(M-1)$, not $M$.} This arises from quotient geometry: alignment is invariant under global rotation, so one phase serves as reference, leaving $M-1$ independent constraints.

For small windows: $p_1(\varepsilon, \kappa) \approx \frac{\varepsilon}{2\pi} \cdot \frac{e^\kappa}{I_0(\kappa)}$. In the \emph{incoherent limit} ($\kappa \to 0$, $r \to 0$): $p_1 \to \varepsilon/(2\pi)$, recovering the uniform geometric probability. In the \emph{coherent limit} ($\kappa \to \infty$, $r \to 1$): $p_1 \to 1$, and alignment is certain.

Assuming independence across modules on the commit timescale, the mean waiting time is:
...
\end{equation}
where $\alpha$ captures \emph{effective independence}: $\alpha = 1$ for fully independent modules; $\alpha < 1$ when inter-module coupling induces correlations that reduce the effective number of constraints. Perturbative analysis shows $\alpha = 1 - \beta(\mathrm{topology}, \kappa_{\mathrm{int}})$ where $\beta \geq 0$ depends on inter-module coupling strength and network topology (a detailed geometric derivation is available~\cite{todd2025ig}).

\textbf{Physical interpretation:} $M$ is the \textbf{coordination depth}---the number of semi-independent modules (locally coherent clusters) whose order parameters must jointly exceed a threshold for a commit to register. $\Delta\omega$ is the \textbf{phase-exploration rate}: the characteristic frequency at which phases mix, combining natural frequency spread and phase diffusion from noise~\cite{ott2008,watanabe1994}. The tolerance $\varepsilon$ is the \textbf{full phase window width} (in radians): alignment requires all module phases to lie within a band of width $\varepsilon$. Note: $M \le \Deff$ where $\Deff$ is the full manifold dimensionality; $M$ counts only commit-relevant modules.

\textbf{Practical approximation.} For moderate coherence ($r \approx 0.3$--$0.8$), a useful approximation for concentration is $\kappa \approx r(2-r^2)/(1-r^2)$~\cite{mardia2000}. The window probability can then be computed numerically via $p_1(\varepsilon, \kappa) = \int_{-\varepsilon/2}^{\varepsilon/2} \frac{e^{\kappa\cos\theta}}{2\pi I_0(\kappa)} d\theta$, or approximated for small $\varepsilon$ as:
\begin{equation}\label{eq:p1-approx}
p_1(\varepsilon, \kappa) \approx \frac{\varepsilon \cdot e^\kappa}{2\pi I_0(\kappa)} \quad \text{(small } \varepsilon \text{)}
\end{equation}
For larger $\varepsilon$ approaching $\pi$, numerical integration is recommended. Representative values: at $r=0.7$ ($\kappa \approx 2.0$), $p_1(\pi, 2.0) \approx 0.62$; at $r=0.8$ ($\kappa \approx 3.0$), $p_1(\pi, 3.0) \approx 0.76$.
...
\paragraph{Visual binding window (computed from Eq.~\ref{eq:coh-time}).}
Take $M=8$ occipital modules (V1, V4, posterior parietal sub-regions), $r=0.7$ (attention~\cite{fries2015}), full tolerance $\varepsilon=\pi$ rad ($\pm 90^\circ$ phase window), and phase-exploration rate $\Delta\omega=2\pi\times 20$ rad/s (alpha--beta spread/phase diffusion).

First compute $p_1$: at $r=0.7$, $\kappa \approx 2.0$ (from $\kappa \approx r(2-r^2)/(1-r^2)$; numerically $r = I_1(2.0)/I_0(2.0) \approx 0.70$), giving $p_1(\pi, 2.0) \approx 0.62$ (numerical integration). Using Eq.~\ref{eq:coh-time} with $\alpha=0.7$ (modular cortical topology):
\[
\tau_{\mathrm{coh}} \approx \frac{1}{125.66}\,(0.62)^{-0.7 \times 7}
= \frac{1}{125.66}\,(0.62)^{-4.9} \approx \frac{11.5}{125.66}\approx 91~\mathrm{ms}.
\]
With higher coherence $r=0.8$ ($\kappa \approx 3.0$, $p_1(\pi, 3.0) \approx 0.76$):
\[
\tau_{\mathrm{coh}} \approx \frac{1}{125.66}\,(0.76)^{-4.9} \approx 35~\mathrm{ms}.
\]

\paragraph{Sensitivity analysis.} The 30--100 ms binding window occupies a robust region of parameter space:

\begin{center}
\begin{tabular}{ccccc|c}
\toprule
$M$ & $r$ & $\varepsilon$ & $\Delta\omega$ (rad/s) & $\alpha$ & $\tau_{\mathrm{coh}}$ (ms) \\
\midrule
6  & 0.7 & $\pi$   & $2\pi \times 20$ & 0.7 & 38 \\
8  & 0.75& $\pi$   & $2\pi \times 20$ & 0.7 & 52 \\
8  & 0.8 & $\pi$   & $2\pi \times 20$ & 0.7 & 35 \\
10 & 0.75& $\pi$   & $2\pi \times 20$ & 0.7 & 72 \\
10 & 0.8 & $\pi$   & $2\pi \times 25$ & 0.7 & 40 \\
12 & 0.8 & $\pi$   & $2\pi \times 20$ & 0.5 & 55 \\
\bottomrule
\end{tabular}
\end{center}

Binding windows of 30--100 ms span plausible ranges of $M \in [6,12]$, $r \in [0.7, 0.8]$, and $\Delta\omega \in [2\pi \times 15, 2\pi \times 25]$ rad/s. The 30--50 ms human binding window under focused attention corresponds to $r \approx 0.8$ (high coherence), moderate coordination depth $M \sim 6$--8.
...
\tau_{\mathrm{coh}} &\approx 35\text{--}90~\mathrm{ms} \quad \text{($r = 0.7$--$0.8$, $M = 8$)}
\end{align}

The effective commit time is $\tau_{\mathrm{eff}} = \max[\text{QSL, SNR, power, coh}] \approx 35$--$90~\mathrm{ms}$, matching human binding windows. \textbf{Coherence time dominates}.

Modest parameter variations produce order-of-magnitude shifts: $r = 0.8$ with $M = 6$ (stronger attention, smaller assembly) yields $\tau_{\mathrm{coh}} \sim 20$ ms; $r = 0.65$ (relaxed coupling) yields $\tau_{\mathrm{coh}} \sim 150$ ms; $M = 12$ (cross-modal audio-visual) yields $\tau_{\mathrm{coh}} \sim 200$ ms, matching cross-modal binding latencies~\cite{vanrullen2016}.
...
\textbf{Mechanistic hypothesis:} We propose dual commit pathways (alternative accounts exist, including attentional sampling~\cite{vanrullen2016} and memory density effects; the key empirical discriminator is predicted independence between temporal-order thresholds and simple RT under arousal):
...
\paragraph{Cross-species conservation.}
Remarkably, empirical data show that cerebellar parallel fiber delays are \emph{conserved} at 5--10~ms across species from mouse to macaque, despite dramatic differences in brain size~\cite{wyatt2005}. This conservation suggests that cerebellar timing is tuned to a biophysically fundamental timescale---likely muscle contraction dynamics at 25--50~Hz---rather than whole-body locomotion frequency, which scales as $M^{-0.15}$ with body mass~\cite{heglund1974}. Larger-brained species (marmoset, cat, macaque) add myelinated parallel fibers in deep cerebellar layers with 0.5--1.0~ms delays, providing an additional fast timing channel. This architecture---conserved slow delays plus optional fast myelinated paths---may enable the cerebellum to simultaneously track both fast muscle dynamics and slower coordination programs.

\paragraph{Low-dimensional systems converge on simple geometry.}
Purkinje cell dendritic arbors exhibit ``fern-leaf'' fractal geometry remarkably similar to botanical ferns---among Earth's oldest vascular plants ($\sim$360 Mya). This convergence reflects a deeper principle: \emph{low-dimensional systems produce similar geometric signatures regardless of why they are low-dimensional}. Ferns are geometrically simple because they are ancient---they evolved before complex developmental programs existed, constraining them to recursive branching rules. The cerebellum is geometrically simple because fast motor coordination \emph{requires} low dimensionality: the low-$M$, high-$r$ regime that enables rapid commits (§4.1) necessarily restricts dynamics to low-dimensional manifolds. Both systems, constrained to low-dimensional operation for entirely different reasons, converge on the same fractal architecture.
...
\subsection{Delay embedding as universal biological motif}

The cerebellar architecture---delay-embedded prediction from partial observations---is not unique to motor control. It represents a general solution to a universal biological problem: how to predict and control dynamics when observations are incomplete and delayed.

\paragraph{The general architecture.}
Any system facing this problem requires:
\begin{enumerate}
\item \textbf{Delayed copies} of incoming signals (creating temporal depth)
\item \textbf{Expansion} into higher-dimensional representation (enabling discrimination)
\item \textbf{Learned prediction} of future states
\item \textbf{Error-driven learning} from prediction failures
\end{enumerate}

The cerebellum implements this with parallel fibers, granule cells, Purkinje cells, and climbing fibers. But the same functional architecture can be implemented with entirely different substrates.

\paragraph{Candidate systems.}
We hypothesize that delay-embedding motifs appear wherever biological systems must predict dynamics from partial observations:

\begin{table}[H]
\centering
\begin{tabular}{p{2.5cm}p{3.5cm}p{3cm}p{2.5cm}}
\toprule
\textbf{System} & \textbf{``Parallel fibers''} & \textbf{Prediction target} & \textbf{Timescale} \\
\midrule
Immune memory & TCR/BCR clones with different activation histories & Pathogen dynamics & Hours--days \\
Development & Morphogen gradients sampled over time & Cell fate trajectories & Hours \\
Circadian & Clock proteins with different half-lives & Environmental cycles & $\sim$24h \\
Chemotaxis & Methylation states (multiple timescales) & Chemical gradients & Seconds \\
Plant tropisms & Auxin redistribution delays & Light/gravity direction & Minutes--hours \\
\bottomrule
\end{tabular}
\end{table}

In each case, the system maintains multiple temporal copies of relevant signals, enabling state reconstruction despite incomplete instantaneous information.

\paragraph{Coherence time as the prediction window.}
The cerebellar delay of 5--10~ms is conserved because it matches the coherence time for neural-speed processes---the window within which predictions must be made and evaluated. Other biological systems would have their own $\tau_{\mathrm{coh}}$ matched to their characteristic dynamics:

\begin{itemize}
\item Bacterial chemotaxis: $\tau_{\mathrm{coh}} \sim 1$--10~s (matching tumble/run alternation)
\item Circadian systems: $\tau_{\mathrm{coh}} \sim$ hours (matching transcription-translation feedback)
\item Immune responses: $\tau_{\mathrm{coh}} \sim$ days (matching clonal expansion timescales)
\end{itemize}

The $\tau^* = 1/(4f)$ relationship generalizes: optimal embedding delay is a quarter-period of the dominant dynamics being predicted, regardless of substrate.

\paragraph{Evolutionary convergence.}
If delay embedding is a universal solution to prediction under partial observability, we expect:
\begin{itemize}
\item Convergent architectures across phylogenetically distant systems facing similar prediction problems
\item Timescale matching: embedding delays should scale with the dynamics being predicted
\item Degradation under timing disruption: systems should fail when temporal structure is perturbed even if amplitude is preserved
\end{itemize}

This reframes the cerebellum not as a specialized motor structure but as the neural instantiation of a universal biological computation. The same mathematics that explains cerebellar timing may explain immune memory, developmental timing, and circadian prediction.
...
\]
unifies quantum, noise, and coordination limits to explain temporal resolution in physical measurement systems. Here $M$ is coordination depth (number of semi-independent modules requiring alignment), $p_1(\varepsilon, \kappa)$ is the single-variable alignment probability (determined by von Mises concentration $\kappa$, which increases with Kuramoto coherence $r$), and $\alpha \in (0,1]$ captures effective independence (reduced by inter-module coupling). The exponent $(M-1)$---not $M$---arises from quotient geometry: one phase serves as reference (see~\cite{todd2025ig} for the full geometric derivation).
...
\bibitem{wyatt2005}
Wyatt, K.D., Bhumbra, G.S., Bhumbra, S.K., Bhumbra, N.S., Sherwood, N. (2005). Speed limits in the cerebellum: constraints from myelinated and unmyelinated parallel fibers. \textit{European Journal of Neuroscience}, 21(8), 2285--2290.

\bibitem{heglund1974}
Heglund, N.C., Taylor, C.R., McMahon, T.A. (1974). Scaling stride frequency and gait to animal size: mice to horses. \textit{Science}, 186(4169), 1112--1113.
...
\subsection*{ESM 3: Proposed Kuramoto network validation protocol}

To empirically validate Eq.~\ref{eq:coh-time}, we propose the following simulation protocol for future work:

\textbf{Setup:} Simulate networks of $N = 500$--2000 coupled phase oscillators:
\[
\frac{d\theta_i}{dt} = \omega_i + \frac{K}{N}\sum_{j=1}^N \sin(\theta_j - \theta_i)
\]
with natural frequencies $\omega_i$ drawn from uniform distribution (width $\Delta\omega$), randomized initial phases, and fourth-order Runge-Kutta integration (timestep $dt = 0.01/\Delta\omega$).

\textbf{Measurement:} For coupling near critical value $K \approx K_c + 0.1K_c$, measure time between transient alignments (order parameter $r(t)$ exceeding threshold $r_\star$ for duration $>\varepsilon/\omega_0$). Test whether alignment time scales as $\langle\tau_{\mathrm{align}}\rangle \propto (\varepsilon/2\pi)^{-\alpha(1-r)(M-1)}$ where $M$ is the number of weakly-coupled clusters (via community detection on phase-coupling adjacency matrix).

\textbf{Predictions:}
\begin{itemize}
\item Topology-dependent $\alpha$: all-to-all $\sim 0.9$, sparse random (Erdős-Rényi, $\langle k\rangle = 20$) $\sim 0.7$, modular (Girvan-Newman) $\sim 0.6$
\item Circular variance $(1-r)$ should provide better fit than $(1-r^2)$ at moderate coherence ($r \approx 0.5$--0.7)
\item Scaling exponent $(M-1)$ should match number of semi-independent modules from community structure
\end{itemize}

Code implementing this protocol will be made available upon publication.
