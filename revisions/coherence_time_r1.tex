...
The justification is straightforward: spike timing is sensitively dependent on surrounding electric fields. Any change in local field potential, no matter how small, shifts when action potentials occur. In neural systems operating near criticality~\cite{beggs2003,shew2013}, small timing differences cascade---the precise moment of a spike determines which downstream neurons fire, which determines subsequent dynamics. Electric fields thus serve as a continuous modulator of the discrete spike events they influence. If this is correct, then oscillators---not spikes---become the fundamental primitive, and the coherence time bounds derived here govern processing rate.

Notably, ephaptic coupling operates differently across brain regions. Cerebellar architecture, with its densely packed parallel fibers and Purkinje cells, supports strong local field effects~\cite{han2018}. Cortical architecture, with its columnar organization and heterogeneous cell types, produces different field geometries. This regional variation in ephaptic coupling strength may explain why cerebellar processing appears exempt from the coordination penalties we attribute to high-$M$ cortical assemblies---the cerebellum may use different coupling mechanisms that bypass the phase-alignment bottleneck.
