...
We model commits as threshold crossings of the order parameter across $M$ semi-independent modules. (By ``commit'' we mean an irreversible registration of distributed state into a low-dimensional outcome that constrains subsequent dynamics---not necessarily a single spike or discrete behavioral choice.)
...
\end{equation}
where $\alpha$ reflects the effective coordination regime (empirical validation in ESM~3: modular networks yield $\hat{\alpha} \approx 0.65$; fully-connected and sparse networks show weak or absent scaling).
...
\item \textbf{Modular topology:} The theory applies when modules are internally coherent but not globally phase-locked. Fully synchronized (all-to-all) or highly fragmented (sparse) regimes fall outside its scope (see ESM~3).
\end{enumerate}
...
Take $M=8$ occipital modules (V1, V4, posterior parietal sub-regions), $r=0.7$ (attention~\cite{fries2015}), full tolerance $\varepsilon=\pi$ rad ($\pm 90^\circ$ across modules), and phase-exploration rate $\Delta\omega=2\pi\times 20$ rad/s (alpha--beta spread/phase diffusion). Using Eq.~\ref{eq:coh-time} with $\alpha=0.9$ (strongly coupled cortical regime):
\[
...
\tau_{\mathrm{coh}} = \frac{1}{\Delta\omega}\left(\frac{2\pi}{\varepsilon}\right)^{\alpha(1-r)(M-1)}
\]
unifies quantum, noise, and coordination limits to explain temporal resolution in physical measurement systems. Here $M$ is coordination depth (number of semi-independent modules requiring alignment), $(1-r)$ is circular variance (phase dispersion), and $\alpha$ reflects the effective coordination regime (topology $\times$ coupling), not topology alone.
...
\]
where $A_{ij}$ encodes modular topology (strong intra-module coupling, weak inter-module coupling), natural frequencies $\omega_i \sim \mathcal{N}(0, \Delta\omega)$, and $\sigma = 0.2$ rad/s$^{1/2}$ provides phase noise. Coordination depth $M$ equals number of modules, varied from 3 to 12. ($\Delta\omega$ is taken as the natural frequency spread; estimating from phase increments yields similar fits.)
...
\textbf{Results:} Fitting $\log\tau_{\mathrm{coh}} = \alpha(1-\bar{r})(M-1)\log(2\pi/\varepsilon) + \mathrm{const}$ across three topologies yields:

\begin{center}
\begin{tabular}{lccc}
\toprule
Topology & $\hat{\alpha}$ & $r^2$ & Interpretation \\
\midrule
Modular & 0.65 & 0.46 & Clear exponential scaling \\
All-to-all & 0.15 & 0.71 & Weak $M$-dependence \\
Sparse & 0.01 & 0.05 & No scaling \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Regime dependence of coherence-time scaling.} The exponential $\tau_{\mathrm{coh}}$ scaling with $M$ emerges \emph{only} in the modular regime. This is not a failure of the theory but a regime-of-validity discovery:
\begin{itemize}
\item \textbf{Modular networks:} Semi-independent modules with weak inter-module coupling---precisely the regime where the theory's assumptions hold. Each module explores phase independently; global alignment requires genuine coincidence across $M$ constraints.
\item \textbf{All-to-all networks:} Global synchronization dominates. High coherence ($r \to 1$) collapses the $(1-r)(M-1)$ exponent, effectively reducing coordination depth to $M_{\mathrm{eff}} \approx 1$. The system pays no combinatorial coordination cost.
\item \textbf{Sparse networks:} Network fragments; modules do not behave as semi-independent phase explorers. The first-passage alignment event the theory models does not occur cleanly. Sparse results ($\hat{\alpha} \approx 0$) should be interpreted as ``outside model scope,'' not as evidence against the scaling law for biological networks.
\end{itemize}

This result strengthens the biological relevance of the framework: neural networks are hierarchically modular, not all-to-all or sparse random. Biology operates precisely in the regime where coordination time becomes a limiting factor.

We fit least-squares to $\log(\mathrm{median}\,\tau_{\mathrm{coh}})$ across $M$; a sweep point is ``valid'' if $\geq 50\%$ of trials reach the alignment criterion within the simulation horizon (50s). The moderate $r^2$ for modular topology is expected: first-passage times are heavy-tailed, and $n = 10$ trials per $M$ yields high variance in median estimates. Preliminary runs with tighter tolerance ($\varepsilon = \pi/3$) show similar positive $M$-scaling ($\hat{\alpha} \approx 0.16$), suggesting robustness; precise $\varepsilon$-dependence validation requires larger samples.
