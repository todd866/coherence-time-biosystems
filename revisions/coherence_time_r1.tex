...
The moderate $r^2$ is expected: first-passage times are heavy-tailed, and $n = 10$ trials per $M$ yields high variance in median estimates. The key results are that (1) $\hat{\alpha}$ falls within the predicted range for modular topology, and (2) the log-linear scaling of $\tau_{\mathrm{coh}}$ with $(M-1)$ is robust across the sweep.
