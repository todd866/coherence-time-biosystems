...
The alignment probability has a precise geometric form. For $M$ independent phases, the probability that all lie within $\varepsilon$ of each other is (see companion paper for rigorous derivation):
...
\end{equation}
where $\alpha$ captures \emph{effective independence}: $\alpha = 1$ for fully independent modules; $\alpha < 1$ when inter-module coupling induces correlations that reduce the effective number of constraints. Perturbative analysis (companion paper) shows $\alpha = 1 - \beta(\mathrm{topology}, \kappa_{\mathrm{int}})$ where $\beta \geq 0$ depends on inter-module coupling strength and network topology.
