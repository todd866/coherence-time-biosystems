...
\textbf{Practical approximation.} For moderate coherence ($r \approx 0.3$--$0.7$), where $\kappa \approx 2r/(1-r^2)$, the window probability can be approximated as:
\begin{equation}\label{eq:p1-approx}
p_1(\varepsilon, r) \approx \frac{\varepsilon}{2\pi}\left(1 + \frac{2r}{1-r}\right)^{1/2}
\end{equation}
This preserves the correct limits: $p_1 \to \varepsilon/(2\pi)$ as $r \to 0$, and $p_1 \to 1$ as $r \to 1$.
...
First compute $p_1$: at $r=0.7$, $\kappa \approx 2.75$ (from $r = I_1(\kappa)/I_0(\kappa)$), giving $p_1(\pi, 2.75) \approx 0.65$ (numerical integration). Using Eq.~\ref{eq:coh-time} with $\alpha=0.7$ (modular cortical topology):
\[
\tau_{\mathrm{coh}} \approx \frac{1}{125.66}\,(0.65)^{-0.7 \times 7}
= \frac{1}{125.66}\,(0.65)^{-4.9} \approx \frac{9.1}{125.66}\approx 72~\mathrm{ms}.
\]
With higher coherence $r=0.8$ ($\kappa \approx 5.3$, $p_1 \approx 0.82$):
\[
\tau_{\mathrm{coh}} \approx \frac{1}{125.66}\,(0.82)^{-4.9} \approx 30~\mathrm{ms}.
\]

\paragraph{Sensitivity analysis.} The 30--50 ms binding window occupies a robust region of parameter space:
...
\midrule
6  & 0.6 & $\pi/2$ & $2\pi \times 10$ & 0.7 & 95 \\
8  & 0.7 & $\pi$   & $2\pi \times 15$ & 0.7 & 48 \\
8  & 0.8 & $\pi$   & $2\pi \times 20$ & 0.7 & 30 \\
10 & 0.7 & $\pi$   & $2\pi \times 20$ & 0.7 & 65 \\
10 & 0.8 & $3\pi/2$& $2\pi \times 25$ & 0.7 & 22 \\
12 & 0.8 & $\pi$   & $2\pi \times 20$ & 0.5 & 45 \\
\bottomrule
\end{tabular}
\end{center}

Binding windows of 20--100 ms span plausible ranges of $M \in [6,12]$, $r \in [0.6, 0.8]$, $\varepsilon \in [\pi/2, 3\pi/2]$, and $\Delta\omega \in [2\pi \times 10, 2\pi \times 25]$ rad/s. The 30--50 ms human binding window corresponds to moderate coordination depth under focused attention.
