...
Our formula is compatible with (and provides a mechanistic instantiation of) Igamberdiev's biological time; it does not require a quantum substrate claim.
...
\boxed{\tau_{\mathrm{coh}} \approx \frac{1}{\Delta\omega}\left(\frac{2\pi}{\varepsilon}\right)^{\alpha\,(1-r)\,(M-1)}},\qquad \alpha\in[0.3,0.6],
\end{equation}
where $\alpha$ reflects the effective coordination regime (empirically validated below: modular networks yield $\hat{\alpha} \approx 0.35$).
...
\]
where $A_{ij}$ encodes modular topology (strong intra-module coupling $K_{\rm intra} = 1.0$, weak inter-module coupling $K_{\rm inter} = 0.15$), natural frequencies $\omega_i \sim \mathcal{N}(0, \omega_{\rm std})$ with $\omega_{\rm std} = 0.3$ rad/s, and $\sigma = 0.1$ rad/s$^{1/2}$ provides phase noise. Coordination depth $M$ (number of modules) varied from 3 to 10. We report $\Delta\omega = \omega_{\rm std}$ for interpretability; using the proxy $\omega_{\rm std} + \sigma^2/2$ yields similar $\hat{\alpha}$.
...
Fitting $\log(\mathrm{median}\,\tau_{\mathrm{coh}}) = \alpha(1-\bar{r})(M-1)\log(2\pi/\varepsilon) + \mathrm{const}$, where $\bar{r}$ is the mean global order parameter over the post-transient window, yields:
\begin{center}
\begin{tabular}{lccc}
\toprule
...
Take $M=8$ occipital modules (V1, V4, posterior parietal sub-regions), $r=0.7$ (attention~\cite{fries2015}), full tolerance $\varepsilon=\pi$ rad ($\pm 90^\circ$ across modules), and phase-exploration rate $\Delta\omega=2\pi\times 20$ rad/s (alpha--beta spread). Using Eq.~\ref{eq:coh-time} with $\alpha=0.6$:
\[
\tau_{\mathrm{coh}} \approx \frac{1}{125.66}\left(\frac{2\pi}{\pi}\right)^{0.6(1-0.7)(7)}
= \frac{1}{125.66}\,2^{1.26}\approx \frac{2.4}{125.66}\approx 20\text{--}40~\mathrm{ms}.
\]
...
\tau_{\mathrm{coh}} &\approx 20\text{--}40~\mathrm{ms}
\end{align}
...
\item Topology dependence: $\alpha$ varies with network structure. We find $\hat{\alpha} \approx 0.35$ for modular networks; other architectures yield different values (Table~1).
\item Nonstationary $\Delta\omega$: During arousal or task transitions, phase-exploration rate may change dynamically.
\item Heavy-tailed first-passage times: Individual coherence events show high variance; the formula describes mean/median behavior. This variance may contribute to trial-by-trial reaction time variability (RTV), a robust behavioral correlate of executive function and attention~\cite{jensen2007}. The framework predicts that RTV should increase with $M$ and decrease with $r$, providing an additional behavioral test.
\end{itemize}
...
\tau_{\mathrm{coh}} = \frac{1}{\Delta\omega}\left(\frac{2\pi}{\varepsilon}\right)^{\alpha(1-r)(M-1)}
\]
unifies quantum, noise, and coordination limits to explain temporal resolution in physical measurement systems. Numerical validation confirms exponential scaling in modular networks ($\hat{\alpha} \approx 0.35$, $R^2 = 0.71$), with the formula's scope limited to hierarchically modular architectures---precisely the regime where neural networks operate.
...
\item \textbf{Dual-loop architecture}: Separate perceptual (high-$M$ cortical) and motor (low-$M$ cerebellar) pathways explain tachypsychia dissociation.
