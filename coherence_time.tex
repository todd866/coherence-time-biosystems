...
\textbf{All-to-all coupling} ($\hat{\alpha} \approx 1$ via linearized surrogate, $n=5$): theory-consistent fit is ill-conditioned in this high-coherence regime where $\bar{r}_{\mathrm{inter}} \approx 1$ makes $-\log p_1 \approx 0$; the surrogate value slightly exceeding 1 reflects linearization error.
\textbf{Sparse random networks} ($\hat{\alpha} \approx 0.1$, $R^2 = 0.28$, $n=4$): strong correlations collapse effective constraints; the theory correctly predicts small $\alpha$ but the linear-in-$M$ model fits poorly, indicating this regime may require nonlinear corrections for quantitative prediction.}
\label{fig:topology}
\end{figure}
...
A scaling argument makes this intuitive. Representational capacity resides in the phase offsets between oscillators. The number of distinguishable configurations scales as $d_{\mathrm{eff}} \sim 1/(f \cdot \tau_{\mathrm{noise}})$, where $\tau_{\mathrm{noise}}$ is the substrate noise floor---fewer phase slots are resolvable per cycle at higher frequency. This suggests dimensionality is inversely proportional to frequency, though a rigorous derivation would require specifying the noise model.
