...
The three topologies span the framework's regime map.
\textbf{Modular networks} ($\hat{\alpha} \approx 1$, $R^2 = 0.97$): near-independent modules, the regime where the theory's assumptions are best satisfied and validation is strongest.
\textbf{All-to-all coupling} ($\hat{\alpha} \approx 1$ via linearized surrogate): theory-consistent fit is ill-conditioned in this high-coherence regime where $\bar{r}_{\mathrm{inter}} \approx 1$ makes $-\log p_1 \approx 0$; the surrogate value slightly exceeding 1 reflects linearization error.
\textbf{Sparse random networks} ($\hat{\alpha} \approx 0.1$, $R^2 = 0.28$): strong correlations collapse effective constraints; the theory correctly predicts small $\alpha$ but the linear-in-$M$ model fits poorly, indicating this regime may require nonlinear corrections for quantitative prediction.}
\label{fig:topology}
\end{figure}
...
Spike trains may participate in coordination (e.g., through spike-field coupling or spike-timing-dependent plasticity), but our framework does not require taking a position on whether spikes or fields are ``fundamental.'' What matters is that \emph{some} continuous coordination variable exists whose alignment determines commit timing. Candidate substrates include local field potentials, ephaptic coupling~\cite{anastassiou2011,pinotsis2023,han2018}, dendritic integration, subthreshold dynamics, and synaptic state variables. The coherence-time framework applies regardless of the specific coupling mechanism. Whether ephaptic coupling is functionally significant---a question that remains open, particularly near criticality~\cite{beggs2003,shew2013}---does not affect the core result: inter-module phase coordination limits processing rate whenever commits require multi-module alignment.
...
\subsection{Frequency and dimensionality}

A prominent view in systems neuroscience holds that high-frequency oscillations (gamma) support high-dimensional computation, while low-frequency oscillations (theta, alpha) provide low-dimensional scaffolding~\cite{miller2018,lundqvist2016,pinotsis2023}. The coherence time framework suggests a different relationship based on coordination constraints.

Consider an oscillatory system that must achieve phase alignment to register a commit. The number of alignment ``attempts'' per cycle is roughly $\lambda_{\mathrm{attempt}} / f$. At high frequency, few attempts are available per cycle, so reliable alignment requires high $p_1$ and thus high coherence $r$---but high $r$ confines dynamics to a low-dimensional synchronized manifold. At low frequency, many attempts per cycle permit low $r$ and thus high-dimensional phase exploration. This is not a correlation but a constraint: \emph{fast oscillations force low dimensionality; slow oscillations permit high dimensionality.}

This can be made precise. Representational capacity resides in the phase offsets between oscillators. The number of distinguishable configurations scales as $d_{\mathrm{eff}} \sim 1/(f \cdot \tau_{\mathrm{noise}})$, where $\tau_{\mathrm{noise}}$ is the substrate noise floor. Dimensionality is inversely proportional to frequency.

The empirical observation that gamma correlates with ``detailed'' processing admits an alternative interpretation: high-frequency gamma may reflect \emph{local}, low-$M$ modules committing rapidly---many parallel low-dimensional commits, not high-dimensional integration. Meanwhile, low-frequency oscillations may coordinate high-$M$ assemblies that \emph{require} slow timescales to achieve alignment. Three predictions distinguish the views: (1) increasing gamma power at fixed $M$ should \emph{decrease} dimensionality (forcing tighter coupling), not increase it; (2) bits per alpha cycle should exceed bits per gamma cycle; (3) gamma bursts should reflect local low-$M$ commits, potentially out of phase with slower coordinating rhythms. These predictions are testable with existing recording and analysis methods.

A related control-theoretic point: Ashby's law of requisite variety~\cite{ashby1956} implies that a subnetwork constrained to a low-dimensional regime loses the degrees of freedom needed to regulate coupled regions. The speed-flexibility trade-off (Proposition~\ref{prop:tradeoff}) is, in this light, a temporal expression of Ashby's constraint: high-$M$ coordination provides requisite variety for flexible regulation but pays an exponential cost in commit time.
...
We have established that coherence time---the time required for distributed oscillatory assemblies to achieve sufficient phase alignment for irreversible state registration---scales exponentially with coordination depth $M$. The unified bound $\tau_{\text{eff}} \geq \max[\tau_{\text{QSL}}, \tau_{\text{SNR}}, \tau_{\text{coh}}, \tau_{\text{power}}]$ shows that coordination, not quantum switching, is the dominant temporal constraint in biological neural systems: coherence times exceed quantum speed limits by eleven orders of magnitude.

The central finding is the speed-flexibility trade-off: increasing $M$ exponentially slows commits but expands combinatorial flexibility, while increasing coupling $r$ speeds commits but restricts dynamics to low-dimensional manifolds. Biological systems navigate this frontier, and the framework makes quantitative predictions for binding windows, metabolic scaling of temporal acuity, and entrainment linearity that are testable with existing methods.
