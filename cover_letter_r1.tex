...
\begin{itemize}
\item \textbf{Phase delta regime} (\S2.3): New section identifying the pre-commit interval---in which structured inter-module phase relationships bias downstream outcomes without discrete state registration---as a computationally productive mode operating below the Landauer threshold. Includes formal Definition~2. The speed-flexibility trade-off (Proposition~2, \S2.2) follows directly.
...
\item \textbf{Alpha oscillations and perception sets} (\S3.3): Integrated recent work by Wutz (2024) showing alpha reflects internally generated ``perception sets.'' We show this is complementary: a perception set can be interpreted as a high-dimensional internal template, and the temporal effects documented---dilation under richer processing, compression under impoverished conditions---correspond to the exponential dependence of $\tau_{\mathrm{coh}}$ on coordination depth.

\item \textbf{Synchronisation theory connections} (\S4.5, \S4.8): Added Buzs\'{a}ki \& Draguhn (2004) for the frequency-hierarchy view (\S4.5). Added connections to the EI-Kuramoto model (Kuroki \& Mizuseki, 2025) and criticality as a computational operating point (Hengen \& Shew, 2025) in the limitations section (\S4.8).
...
\medskip
\textbf{Data and code availability.} All simulation code and parameter estimation workflows are available at \url{https://github.com/todd866/coherence-time-biosystems}.
